# 家庭化学品排雷大挑战——移动端视觉基础约束

> 版本：v1.1（非最终视觉稿）
> 日期：2026-07-16
> 上游输入：[用户流程](../../docs/product/用户流程.md) · [手机端页面线框图](../../docs/design/手机端页面线框图.md)
> 可视化样式板：[打开浏览器样式板](./mobile-style-guide.html)

![移动端视觉规范样式板](./mobile-style-guide.png)

## 当前定位

本文件不是最终美术定稿。编码前只确认会影响组件结构和可访问性的基础约束：

- 原始令牌、语义令牌、组件令牌三层结构。
- 文字与控件的 WCAG AA 对比度目标。
- 系统字体、字号层级和 4pt 间距体系。
- 按钮、输入和触控区域的最低尺寸。
- 风险状态必须同时使用文字、图标和颜色。

以下内容留到功能编码和真实页面联调完成后决定：
...
当前橙色样式板是可访问的候选方向，用于验证令牌落地，不代表必须采用该美术风格。
...
- [x] 使用“原始令牌 → 语义令牌 → 组件令牌”三层结构。
- [x] 关键文字与控件达到 WCAG AA 对比度目标。
- [x] 使用系统中文字体和明确的字号/行高层级。
- [x] 使用 4pt 间距体系和不低于 44×44 的触控区域。
- [x] 风险状态始终搭配文字、图标和颜色。
- [x] 业务组件不直接写最终色值，只引用语义令牌。
- [x] 最终色板、插画、图标、阴影与动效留到功能完成后决定。
- [x] 下一阶段先画组件图与令牌映射，再进入功能编码。

下一份产物是《前端组件图与设计令牌映射》。功能编码完成后，再回到本文件完成最终视觉定稿。
