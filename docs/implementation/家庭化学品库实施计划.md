...
详细方案见 [Android 演示版落地方案](../deployment/Android演示版落地方案.md)。
...
实施计划已于 2026-08-02 确认。P0 至 F6 全部切片已完成编码和自动化验证，Q1 自动化检查通过（后端 165 项 + 移动端 128 项测试、TypeScript、Expo Web 导出）。

2026-08-06 更新：真机 QA 已完成。Android 演示版 APK（v1.0.2 / versionCode 3，EAS 云端构建）在真机（红米 K90）完成"拍照识别—核对入库—查看—编辑—删除"全流程验证。期间修复 Android release 构建下 RN fetch multipart 上传静默失败问题（改用 `FileSystem.uploadAsync`，见 `mobile/src/services/recognitionUpload.ts` 及对应测试）。
